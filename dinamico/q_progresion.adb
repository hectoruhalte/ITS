...
package body Q_PROGRESION is
   ...
   begin
      ...
      if Q_RUTA.Q_LISTA_TRAMOS.F_CUANTOS_ELEMENTOS (V_RUTA) = 1 then
         ...
         loop
            ...
            -- Inicializar la lista de cruces del elemento de progresion.
            Q_CONEXION.Q_LISTA_CRUCES.P_INICIALIZAR_LISTA (V_ELEMENTO_PROGRESION.R_LISTA_CRUCES);
         ...
         loop
            ...
            if V_POSICION_TRAMO_ACTUAL = Q_RUTA.Q_LISTA_TRAMOS.F_CUANTOS_ELEMENTOS (V_RUTA) then				
               ...
               -- Inicializar la lista de cruces del elemento de progresion.
               -- No estamos en una conexion. La list de cruces debe estar vacia.
               Q_CONEXION.Q_LISTA_CRUCES.P_INICIALIZAR_LISTA (V_ELEMENTO_PROGRESION.R_LISTA_CRUCES);
               ...
               -- Inicializar la lista de cruces.
               -- No estamos en una conexion. La lista de cruces debe estar vacia.
               Q_CONEXION.Q_LISTA_CRUCES.P_INICIALIZAR_LISTA (V_ELEMENTO_PROGRESION.R_LISTA_CRUCES);
               ...
               if Q_TIPOS_BASICOS."=" 
                 ...
                 Q_TIPOS_BASICOS."="
                  ...
                  if V_POSICION_SEGMENTO = 0 then
                     ...
                     -- Ya tenemos el tramo actual, el tramo siguiente, el carril actual y el carril siguiente.
                     -- Hay que obtener con esos datos la lista de cruces y a�adirlos al elemento de progresion.
                     V_ELEMENTO_PROGRESION.R_LISTA_CRUCES := 
                       Q_TRAMO.F_OBTENER_LISTA_CRUCES 
                         (V_TRAMO_ACTUAL       => V_TRAMO_ACTUAL,
                          V_CARRIL_ACTUAL      => 
                            F_OBTENER_CARRIL_OPTIMO (V_TRAMO_ID                   => Q_TRAMO.F_OBTENER_ID (V_TRAMO_ACTUAL),
                                                     V_PROGRESION_CARRILES_OPTIMO => V_PROGRESION_CARRILES_OPTIMO),
                          V_TRAMO_SIGUIENTE_ID => V_TRAMO_SIGUIENTE_ID,
                          V_CARRIL_SIGUIENTE   => F_OBTENER_CARRIL_OPTIMO (V_TRAMO_ID                   => V_TRAMO_SIGUIENTE_ID,
                                                                           V_PROGRESION_CARRILES_OPTIMO => V_PROGRESION_CARRILES_OPTIMO));
                  ...
                  for K in reverse 1 .. F_CUANTOS_ELEMENTOS (V_PROGRESION_CARRILES_OPTIMO) loop
                     ...
                     then
                        ...
                        then
                           ...
                           -- Comprobar si el tramo de entrada a la rotonda tiene 2 carriles
                           if Q_ADAPTACION_TRAMO.F_NUMERO_CARRILES_ULTIMO_SEGMENTO
                             (F_DEVOLVER_ELEMENTO
                                (V_POSICION => K,
                                 V_LISTA =>
   ...
   function "=" (V_ELEMENTO_SENTIDO_CIRCULACION_1 : in T_ELEMENTO_SENTIDO_CIRCULACION;
                 V_ELEMENTO_SENTIDO_CIRCULACION_2 : in T_ELEMENTO_SENTIDO_CIRCULACION) return Boolean is
      
   begin
      
      return V_ELEMENTO_SENTIDO_CIRCULACION_1.R_ID_TRAMO_ACTUAL = V_ELEMENTO_SENTIDO_CIRCULACION_2.R_ID_TRAMO_ACTUAL;
   ...
   procedure P_GENERAR_PROGRESION_SENTIDO_CIRCULACION (V_POSICION_ORIGEN : in Q_TIPOS_BASICOS.T_POSICION_UTM;
                                                       V_POSICION_FINAL : in Q_TIPOS_BASICOS.T_POSICION_UTM;
                                                       V_RUTA : in Q_RUTA.T_RUTA;
                                                       V_PROGRESION_SENTIDO_CIRCULACION : out T_LISTA_SENTIDO_CIRCULACION) is
      
      V_SEGMENTO_ORIGEN_AUX, V_SEGMENTO_FINAL_AUX : Q_SEGMENTO.T_SEGMENTO;
      
      V_LISTA_SEGMENTOS : Q_TRAMO.Q_LISTA_SEGMENTOS.T_LISTA;
      
      V_TRAMO, V_TRAMO_SIGUIENTE : Q_TRAMO.T_TRAMO;
      
      V_TRAMO_ID, V_TRAMO_SIGUIENTE_ID : Natural := 0;
      
      V_CONEXION : Q_TIPOS_BASICOS.T_POSICION_UTM;
      
   begin
      
      P_INICIALIZAR_LISTA (V_PROGRESION_SENTIDO_CIRCULACION);
      
      if Q_RUTA.Q_LISTA_TRAMOS.F_CUANTOS_ELEMENTOS (V_RUTA) = 1 then
         
         -- Solo hay un tramo en la ruta
         
         -- Establecer las posiciones de los segmentos auxiliares
         Q_SEGMENTO.P_PONER_POSICION (V_POSICION => V_POSICION_ORIGEN,
                                      V_SEGMENTO => V_SEGMENTO_ORIGEN_AUX);
         Q_SEGMENTO.P_PONER_POSICION (V_POSICION => V_POSICION_FINAL,
                                      V_SEGMENTO => V_SEGMENTO_FINAL_AUX);
         
         -- Extraer la lista de segmentos
         Q_TRAMO.P_INICIALIZAR_TRAMO (V_TRAMO);
         V_TRAMO := Q_RUTA.Q_LISTA_TRAMOS.F_DEVOLVER_ELEMENTO (V_POSICION => 1,
                                                               V_LISTA    => V_RUTA);
         Q_TRAMO.Q_LISTA_SEGMENTOS.P_INICIALIZAR_LISTA (V_LISTA_SEGMENTOS);
         
         V_LISTA_SEGMENTOS := Q_TRAMO.F_OBTENER_LISTA_SEGMENTOS (V_TRAMO);
         
         V_TRAMO_ID := Q_TRAMO.F_OBTENER_ID (V_TRAMO);
         
         -- Solo hay un tramo en la lista. El comienzo y el final del trayecto estan en el mismo tramo.
         -- Hay que ver si el tramo se recorre en sentido ascendente (del comienzo hacia el final) o descendente (del final al comienzo)
         if Q_TRAMO.Q_LISTA_SEGMENTOS.F_POSICION_ELEMENTO (V_ELEMENTO => V_SEGMENTO_ORIGEN_AUX,
                                                           V_LISTA    => V_LISTA_SEGMENTOS) >
            Q_TRAMO.Q_LISTA_SEGMENTOS.F_POSICION_ELEMENTO (V_ELEMENTO => V_SEGMENTO_FINAL_AUX,
                                                           V_LISTA    => V_LISTA_SEGMENTOS)
         then
            
            -- Sentido ascendente. Obtener el sentido de la adaptacion.
            P_INSERTAR_ELEMENTO 
              (V_ELEMENTO => 
                 T_ELEMENTO_SENTIDO_CIRCULACION'
                   (R_ID_TRAMO_ACTUAL => V_TRAMO_ID,
                    R_SENTIDO_ACTUAL  => Q_ADAPTACION_TRAMO.F_OBTENER_SENTIDO_CIRCULACION (V_TRAMO_ID => V_TRAMO_ID,
                                                                                           V_SENTIDO  => Q_ADAPTACION_TRAMO.E_ASCENDENTE)),
               V_LISTA    => V_PROGRESION_SENTIDO_CIRCULACION);
            ...
            P_INSERTAR_ELEMENTO 
              (V_ELEMENTO => 
                 T_ELEMENTO_SENTIDO_CIRCULACION'
                   (R_ID_TRAMO_ACTUAL => V_TRAMO_ID,
                    R_SENTIDO_ACTUAL  => Q_ADAPTACION_TRAMO.F_OBTENER_SENTIDO_CIRCULACION (V_TRAMO_ID => V_TRAMO_ID,
                                                                                           V_SENTIDO  => Q_ADAPTACION_TRAMO.E_DESCENDENTE)),
               V_LISTA    => V_PROGRESION_SENTIDO_CIRCULACION);
         ...
         -- Hay al menos dos tramos en la ruta
         -- Recorrer los tramos de la ruta hasta el ultimo, que se obtiene aparte
         for I in 1 .. Q_RUTA.Q_LISTA_TRAMOS.F_CUANTOS_ELEMENTOS (V_RUTA) - 1 loop
         
            -- Hay que averiguar si en cada tramo de la ruta vamos del origen al final del tramo o viceversa
            -- Para ello: Se obtiene el punto de conexion entre el tramo actual y el siguiente, si esa conexion corresponde al destino
            -- entonces el tramo se recorres en modo ascendente, si esa conexion corresponde al origen del tramo entonces el tramo se
            -- recorre en modo descendente.
            
            -- Obtener el id del tramo actual y del siguiente.
            Q_TRAMO.P_INICIALIZAR_TRAMO (V_TRAMO);
            Q_TRAMO.P_INICIALIZAR_TRAMO (V_TRAMO_SIGUIENTE);
            
            V_TRAMO := Q_RUTA.Q_LISTA_TRAMOS.F_DEVOLVER_ELEMENTO (V_POSICION => I,
                                                                  V_LISTA    => V_RUTA);
            
            V_TRAMO_ID := Q_TRAMO.F_OBTENER_ID (V_TRAMO);
            
            V_TRAMO_SIGUIENTE := Q_RUTA.Q_LISTA_TRAMOS.F_DEVOLVER_ELEMENTO (V_POSICION => I+1,
                                                                            V_LISTA    => V_RUTA);
            
            V_CONEXION := Q_ADAPTACION_TRAMO.F_OBTENER_CONEXION (V_TRAMO_1 => V_TRAMO,
                                                                 V_TRAMO_2 => V_TRAMO_SIGUIENTE);
            
            if Q_TIPOS_BASICOS."=" (V_POSICION_1 => V_CONEXION,
                                    V_POSICION_2 => Q_TRAMO.F_OBTENER_ORIGEN (V_TRAMO)) then
               ...
               P_INSERTAR_ELEMENTO 
                 (V_ELEMENTO => 
                    T_ELEMENTO_SENTIDO_CIRCULACION'
                      (R_ID_TRAMO_ACTUAL => V_TRAMO_ID,
                       R_SENTIDO_ACTUAL  => 
                         Q_ADAPTACION_TRAMO.F_OBTENER_SENTIDO_CIRCULACION (V_TRAMO_ID => V_TRAMO_ID,
                                                                           V_SENTIDO  => Q_ADAPTACION_TRAMO.E_DESCENDENTE)),
                  V_LISTA    => V_PROGRESION_SENTIDO_CIRCULACION);
         ...
         -- Obtener el sentido de circulacion para el ultimo tramo de la ruta.
         -- Obtener la conexion entre este ultimo tramo y el anterior. Si la conexion se hace con el origen del ultimo tramo, el sentido
         -- sera ascendente, si se hace con el final, sera descendente
         Q_TRAMO.P_INICIALIZAR_TRAMO (V_TRAMO);
         
         V_TRAMO := Q_RUTA.Q_LISTA_TRAMOS.F_DEVOLVER_ELEMENTO (V_POSICION => Q_RUTA.Q_LISTA_TRAMOS.F_CUANTOS_ELEMENTOS (V_RUTA),
                                                               V_LISTA    => V_RUTA);
         
         V_TRAMO_ID := Q_TRAMO.F_OBTENER_ID (V_TRAMO);
         
         V_CONEXION := 
           Q_ADAPTACION_TRAMO.F_OBTENER_CONEXION 
             (V_TRAMO_1 => V_TRAMO,
              V_TRAMO_2 => Q_RUTA.Q_LISTA_TRAMOS.F_DEVOLVER_ELEMENTO (V_POSICION => Q_RUTA.Q_LISTA_TRAMOS.F_CUANTOS_ELEMENTOS (V_RUTA)-1,
                                                                      V_LISTA    => V_RUTA));
         ...
         if Q_TIPOS_BASICOS."=" (V_POSICION_1 => V_CONEXION,
            ...
            -- Ascendente
            P_INSERTAR_ELEMENTO 
              (V_ELEMENTO => T_ELEMENTO_SENTIDO_CIRCULACION'
                 (R_ID_TRAMO_ACTUAL => V_TRAMO_ID,
                  R_SENTIDO_ACTUAL  => Q_ADAPTACION_TRAMO.F_OBTENER_SENTIDO_CIRCULACION (V_TRAMO_ID => V_TRAMO_ID,
                                                                                         V_SENTIDO  => Q_ADAPTACION_TRAMO.E_ASCENDENTE)),
               V_LISTA    => V_PROGRESION_SENTIDO_CIRCULACION);
   ...
   end P_GENERAR_PROGRESION_SENTIDO_CIRCULACION;
   --------------------------------------------------------------------------------------------------------------------------
   
   ----------------------------------------------------------------------------------------------------------------------------
   function F_OBTENER_SENTIDO_CIRCULACION (V_TRAMO_ID : in Natural;
                                           V_PROGRESION_SENTIDO_CIRCULACION : in T_LISTA_SENTIDO_CIRCULACION) return Natural is
      
      V_ELEMENTO_SENTIDO_CIRCULACION : T_ELEMENTO_SENTIDO_CIRCULACION;
      
   begin
      
      V_ELEMENTO_SENTIDO_CIRCULACION.R_ID_TRAMO_ACTUAL := V_TRAMO_ID;
      
      V_ELEMENTO_SENTIDO_CIRCULACION := F_ENCONTRAR_ELEMENTO (V_ELEMENTO => V_ELEMENTO_SENTIDO_CIRCULACION,
                                                              V_LISTA    => V_PROGRESION_SENTIDO_CIRCULACION);
      
      return V_ELEMENTO_SENTIDO_CIRCULACION.R_SENTIDO_ACTUAL;
      
   end F_OBTENER_SENTIDO_CIRCULACION;
   ----------------------------------------------------------------------------------------------------------------------------
   
   ------------------------------------------------------------------------------------------------------------------------------
   function F_OBTENER_LISTA_CRUCES (V_ELEMENTO_PROGRESION : in T_ELEMENTO_PROGRESION) return Q_CONEXION.Q_LISTA_CRUCES.T_LISTA is
      
   begin
      
      return V_ELEMENTO_PROGRESION.R_LISTA_CRUCES;
